...
const ToggleCheckboxList: React.FunctionComponent<Props> = ({
  ...
  const onRowClick = (heading: string, subheading: string, item: string, parentIdx: number) => {
    let newActiveList = [];
    let activeCount = 0;
    ...
    if (activeRows.includes(item)) {
      newActiveList = activeRows.filter((row) => row !== item);
    ...
    return rows.map((item) => {
      ...
      return (
        <PanelRow key={uniqueId} mods={classes}>
          <p className="u-padLeftLg">{item}</p>
    ...
    return list.map((item, idx) => {
      ...
      return (
        <React.Fragment key={`${headerName}`}>
          ...
          <div ref={bodyRefs.current[idx]} className="Panel-body-wrapper Panel-body--closed">
  ...
  const buttonLabel = activeRows.length === getFullChildCount(list) ? 'Clear All' : 'Select All';
